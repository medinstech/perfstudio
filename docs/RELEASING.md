...
It builds and smoke-tests all three installers *and* the PyPI distributions, and
publishes nothing. **Do not skip this.** The tag is the only other thing that runs those jobs, and a tag that fails
...
**5a. Before the FIRST release that publishes to PyPI**, the trusted publisher has to
exist — see [PyPI](#pypi) below. A tag whose upload is refused has already spent its
version number.
...
Pushing a `v*` tag runs `.github/workflows/release.yml`, which builds four distributions
and publishes them:

| | artefact | built on |
|---|---|---|
| PyPI | `perfstudio-X.Y.Z-py3-none-any.whl` and the sdist | `ubuntu-latest` |
...
The `pypi` job builds an sdist and a pure-Python wheel, checks them, installs the wheel
with its dependencies into a clean virtualenv and runs `perfstudio --version` out of it.
Everything up to that point runs on a dry run too; only the upload is gated on the tag.

It does **not** wait for the three installer jobs. A wheel has different failure modes
from a PyInstaller bundle, and hanging it off ninety minutes of Qt packing would mean a
broken sdist is the last thing anybody hears about. It also means the wheel can still be
published when an installer fails, which is the right way round — the wheel is what a
platform with no bundle falls back to, and `ui/updater.py` already sends a `pip` install
...
itself needs no configuration — it exists so the publish shows up in the repository's
deployment history and so required reviewers can be added later without touching the
workflow.

**There is no TestPyPI rehearsal, and that is a decision.** It needs a second trusted
publisher on a second index and proves nothing the dry run does not, except that an
upload succeeds — which is precisely the step that cannot be rehearsed anyway, because
PyPI never releases a version number back once it has been used. A wheel published by
mistake can be yanked; the number stays spent.

Two things the dry run checks that are easy to get wrong and invisible until published:
`twine check --strict`, because a `long_description` that does not render is a warning by
default and a permanently ugly project page in practice; and the contents of the wheel
against `[tool.setuptools.package-data]`, because that list is written out file by file
on purpose and a list nothing checks is a glob that stops matching when an icon is
renamed.
