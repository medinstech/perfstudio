...
Three things have to hold:
  ...
  2. It must render OFFSCREEN, because the build guide has to generate a per-step image
     from a CLI with no window.
  3. A solder trace must look different from a wire — beads at every pad versus fillets
     only at the ends — because that difference is the model's central claim.

Axis note: rows grow downward in board space (screen-like), so 3D uses y = -row*pitch.
Looking down +Z then matches the 2D editor's orientation instead of mirroring it.
"""
from __future__ import annotations

import vtk

from board_model import Board, Conductor, Document
...
def _xy(board: Board, col: int, row: int) -> tuple[float, float]:
    return col * board.pitch, -row * board.pitch
...
def build_component(doc: Document, comp, board: Board) -> vtk.vtkActor | None:  # noqa: ANN001
    fp = doc.footprint(comp.footprint_id)
    if fp is None:
        return None
    pts = comp.outline_mm(fp, board)
    xs = [p[0] for p in pts]
    ys = [p[1] for p in pts]
    cx = (min(xs) + max(xs)) / 2
    cy = -(min(ys) + max(ys)) / 2
...
def build_conductor(cond: Conductor, board: Board) -> list[vtk.vtkActor]:
    ...
    for i, (col, row) in enumerate(cond.path):
        x, y = _xy(board, col, row)
    ...
    is_trace = cond.contacts_every_hole
    ...
    actor = vtk.vtkActor()
    actor.SetMapper(mapper)
    rgb = _hex_rgb(cond.color, SOLDER_RGB if is_trace else BARE_RGB)
...
def build_renderer(doc: Document, flipped: bool = False) -> tuple[vtk.vtkRenderer, dict]:
    board = doc.board
    ren = vtk.vtkRenderer()
    ...
    for comp in doc.components:
        actor = build_component(doc, comp, board)
        if actor:
            ren.AddActor(actor)
...
def render_offscreen(doc: Document, path: str, width: int = 1400, height: int = 950,
                     flipped: bool = False) -> dict:
    """Headless render. This is the path the build guide's step images would use."""
    ren, stats = build_renderer(doc, flipped=flipped)
